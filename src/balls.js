...
export function createBall(x, rowIndex, phase = 0) {
  const mesh = createBallMesh();
  mesh.position.set(x, R, 0);
  const ball = {
    x, row: rowIndex, mesh, phase,
    collected: false,
    update(t) {
      ...
      mesh.position.y = R + BALL.BOUNCE_HEIGHT * s;
      // Squash for ~0.08 s around contact.
      const nearGround = Math.min(1, (BALL.BOUNCE_HEIGHT * s) / 0.06);
